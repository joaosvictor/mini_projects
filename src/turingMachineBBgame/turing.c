...
/*
 ...
 a0 -> h1r
 Turing Machine for 2-state BB:
 a0 -> b1r    a1 -> b1l
 b0 -> a1l    b1 -> h1r
 Turing Machine for 3-state BB:
 a0 -> b1r    a1 -> h1r
 b0 -> c0r    b1 -> b1r
 c0 -> c1l    c1 -> a1l
 Turing Machine for 4-state BB:
 a0 -> b1r    a1 -> b1l
 b0 -> a1l    b1 -> c0l
 c0 -> h1r    c1 -> d1l
 d0 -> d1r    d1 -> a0r
...
headpos = tapelen / 2;
printf("\nInitial tape; note that head position at each step is marked by ^\n\n");
printtape();
...
do {
 ...
 int incdec = (shift == '0') ? -1 : +1; // shift headposition left or right after next write 

 
//sanity check
if ((headpos < LWBTAPE-1) || (headpos == UPBTAPE - 2)) 
	{printf ("tape too short and TM may be looping\n"); return(1);}
#ifdef DEBUG
